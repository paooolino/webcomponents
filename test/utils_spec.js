...
describe('Action utils', () => {
    ...
    const actionSuccess = (serverData) => ({ type: 'ACTION_SUCCESS', ...serverData });
    ...
    describe('createAsyncAction dispatch', () => {
        ...
        it('calls the SUCCESS action when the response status is ok, and pass whatever data returned', () => {
            nock(ENDPOINT_HOST).post(ENDPOINT_PATH)
                .reply(200, { status: 'ok', whatever_attribute: 'Whatever value' } );
            ...
            return store.dispatch(asyncAction)
                .then(() => {
                    ...
                    expect(actions[1].whatever_attribute).toBe('Whatever value');
                });
        });
